#!/usr/bin/env python3
"""Generate the MindRL workbook notebook."""
import json
...
D = chr(34) * 3  # triple double-quote, avoids conflicts

# ============================================================
# TITLE
# ============================================================
md("# MindRL Challenge: Cognitive Modeling Workbook\n"
   "\n"
...
# DATA
# ============================================================
md("## 2. The Data: 4-Armed Bandit\n"
   "\n"
   "The MindRL Challenge provides human behavioral data from a 4-armed bandit task.\n"
   "Each trajectory is one person playing 120 trials. We convert JSONL to a DataFrame.\n"
   "\n"
   "**Key columns:**\n"
   ...
   "- `participant_id`: which person (for hierarchical modeling)\n"
...
md("Each row is one trial. The person chose an arm (`response`), got a reward\n"
   ...
   "changes over time.\n"
   "\n"
   "The `participant_id` is used for hierarchical modeling: we estimate a\n"
   "population-average parameter for everyone, plus individual adjustments per\n"
   "person. This is more realistic than assuming everyone is identical.")

# ============================================================
# MODEL 1: RW
# ============================================================
md("## 3. Model 1: Rescorla-Wagner (the simplest learner)\n"
   "\n"
...
# MODEL 2: DUAL ALPHA
# ============================================================
md("## 4. Model 2: Dual-Alpha (asymmetric learning)\n"
   "\n"
...
md("With `alpha_pos=0.2` (slow) and `alpha_neg=0.7` (fast), the dual-alpha model\n"
...
# MODEL 3: STICKY
# ============================================================
md("## 5. Model 3: Dual-Alpha + Sticky (the submission model)\n"
   "\n"
...
code("class NArmRWDualAlphaSticky:\n"
     "    def __init__(self, n_actions=4, initial_q=0.5, feedback_field='feedback'):\n"
     "        self._n_actions = n_actions\n"
     "        self._initial_q = initial_q\n"
     "        self._feedback_field = feedback_field\n"
...
# SOFTMAX
# ============================================================
md("## 6. The Decision Rule: Softmax\n"
   "\n"
...
code("def softmax_logprob(q_values, beta, chosen_arm):\n"
...
md("The `max_val` subtraction is a numerical stability trick: `exp(beta * Q)` can\n"
...
code("PRIOR_SPECS = {\n"
     ...
     "print('  formula: param ~ 1 + (1|participant_id)')\n"
     "print('  link:    log_logit (maps unconstrained to bounded)')")

md("The formula `param ~ 1 + (1|participant_id)` means:\n"
   "- Estimate a population intercept (the `1`) -- the group-average value\n"
   "- Plus a random effect per participant (the `(1|participant_id)`) -- individual\n"
...
code("from ssms.rl import ModelConfig\n"
...
md("`ModelConfig` tells HSSM how to combine three pieces:\n"
...
code("# Split data into training and validation with disjoint subjects\n"
     ...
     "        ['participant_id', 'trial_id', 'response', 'feedback']].reset_index(drop=True)\n"
     "\n"
...
md("**R-hat** tells us if the 2 chains agreed. If max R-hat < 1.01, the sampling\n"
...
# NLL SCORING
# ============================================================
md("## 8. Scoring: How Good Are the Predictions?\n"
   "\n"
...
code("def heldout_nll(idata, valid_data, learner_factory, params, bounds, n_draws=100):\n"
     "    posterior = idata.posterior\n"
     "    if hasattr(posterior, 'to_dataset'):\n"
     "        posterior = posterior.to_dataset()\n"
     "    posterior = posterior.stack(sample=('chain', 'draw'))\n"
...
import json
with open("research/mindrl_workbook.ipynb", "w") as f:
